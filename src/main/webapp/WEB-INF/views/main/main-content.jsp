...
<div class="recentcontainer">
                    ...
                    <div class="newpostfooter">
                        <a href="javascript:void(0)">
                          <label id="img-label"><i class="fa fa-image"></i>사진/동영상</label>
                        </a>
                        <a href="javascript:void(0)">
                  ...
                  <c:forEach var="mainBoard" items="${boards}">
	                  <div class="recentcontainer">
	                    <div class="newpostheader">
	                      <a href="profile.do?userNo=${mainBoard.user.no }">
	                        <img src="resources/image/${mainBoard.user.profile }" style="border-radius:50%"/>
	                        <span class="name">${mainBoard.user.name }</span>
	                      </a>
				           <c:if test="${mainBoard.share.no ne null}">
								<span>님이
								<a href="#">게시물</a>
					                 을 공유했습니다.</span>
				           </c:if>
	                      <p>
	                        <a class="date" href="javascript:void(0)">${mainBoard.passedTime }</a>
	                        <a href="javascript:void(0)">
	                    ...
	                    <div class="newpost">
	                      <div class="postcontent" id="post-board-content-${mainBoard.no }">
	                      	<div style="margin-left:10px;margin-top:10px;">
	                        	${mainBoard.contents }
	                        </div>
	                        <div style="height:40px;">
	                        	<div style="margin-left:10px;margin-top:10px;">
	                        	<c:forEach var="tag" items="${mainBoard.tags }">
	                        		<c:if test="${tag.contents ne null}">
		                        		<a href="profile.do?userNo=${tag.user.no }" style="text-decoration: none;">${tag.contents }</a>
	                        		</c:if>
	                        	</c:forEach>
	                        	<c:if test="${not empty mainBoard.tags }">
	                        		과 함께
	                        	</c:if>
	                        	</div>
	                        </div>
	                        <div>
	                      		<c:if test="${not empty mainBoard.boardMedias}">
	         	  	             	<c:forEach var="mainBoardMedia" items="${mainBoard.boardMedias }">
		                 ...
		                 <c:if test="${mainBoard.share.no ne null }">
		                     ...
		                     <div class="community-counter">
		                        <a href="javascript:void(0)" id="likeCount-${mainBoard.no }" data-toggle='tooltip' data-placement='top' data-html=true;>
		                          <i class="fa fa-thumbs-up"></i>
		                          <span id="likeCountInfo-${mainBoard.no} ">${mainBoard.like }</span>
		                        </a>
		                        <a href="javascript:void(0)" id="replyCount-${mainBoard.no }" data-toggle='tooltip' data-placement='top' data-html=true;>
		                          <i class="far fa-comment-alt"></i>
		                          <span id="boardRepCount-${mainBoard.no }">${mainBoard.repCount }</span>
		                        </a>
		                    </div>
		                    <div class="newpostfooter">
		                       <div class="newpostfooter-contents">
		                       <!-- 좋아요를 눌렀는지아닌지에대한 choose -->
			                        <a href="javascript:void(0)" id="likeBoardButton-${mainBoard.no }" style="color:${empty mainBoard.event ? '' : 'Dodgerblue' }">
			                          <label class="board-label"><i class="fa fa-thumbs-up"></i>좋아요</label>
			                        </a>
		                        <a href="javascript:void(0)" id="replyButton-${mainBoard.no }">
		                          <label class="board-label"><i class="far fa-comment-alt"></i>댓글달기</label>
		                        </a>
		                        <a id="shareButton-${mainBoard.no }" href="#" data-toggle="modal" data-target="#shareModal">
		                          <label class="board-label"><i class="fa fa-share"></i>공유하기</label>
		                        </a>
		                        </div>
		                      </div>
		                      <div id="commentListDiv-${mainBoard.no }">
			...
			<div class="modal-dialog modal-center modal-overall">
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="row text-row">
							<div class="postcontent" id="modal-board-content2">
								<div class="recentcontainer">
				                    <div class="newpost mBottom10" id="boardModalTextAreas">
					...
					<div class="modal-footer">
						<div class="newpostfooter">
				        	<a href="javascript:void(0)">
				               	<input type="file" id="img-board" name="boardimgfile" >
				               	<label id="img-board-label" for="img-board"><i class="fa fa-image"></i>사진 / 동영상</label>
				            </a>
