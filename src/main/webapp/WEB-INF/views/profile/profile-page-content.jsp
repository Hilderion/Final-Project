    ...
    pageEncoding="UTF-8"%>
		<div class="row margin20">
            <div class="col-xs-4">
                <div class="information">
                    <p>소개</p>
						<p>페이지 만든이: <span>${user.name }</span></p>
						<p>페이지 카테고리: <span>${page.category }</span></p>
            ...
            <div class="col-xs-6">
                <div class="recentcontainer">
                  ...
                  <c:forEach var="profilePageBoard" items="${pageBoards }">
	                  <div class="recentcontainer">
	                    <div class="newpostheader">
	                      <a href="profile.do?userNo=${profilePageBoard.user.no }">
	                        <img src="resources/image/${profilePageBoard.user.profile }"  style="border-radius:50%"/>
	                        <span class="name">${profilePageBoard.user.name }</span>
	                      </a>
				           <c:if test="${profilePageBoard.share.no ne null}">
								<span>님이
								<a href="#">게시물</a>
					                 을 공유했습니다.</span>
				           </c:if>
	                      <p>
	                        <a class="date" href="javascript:void(0)"><fmt:formatDate value="${profilePageBoard.createDate }" pattern="yyyy년MM월dd일" /></a>
	                        <a href="javascript:void(0)">
	                    ...
	                    <div class="newpost">
	                      <div class="postcontent" id="post-board-content-${profilePageBoard.no }">
	                      	<div style="margin-left:10px;margin-top:10px;">
	                        	<p>${profilePageBoard.contents }</p>
	                        </div>
	                        <div style="height:40px;">
	                        	<div style="margin-left:10px;margin-top:10px;">
	                        	<c:forEach var="tag" items="${profilePageBoard.tags }">
	                        		<c:if test="${tag.contents ne null}">
		                        		<a href="profile.do?userNo=${tag.user.no }" style="text-decoration: none;">${tag.contents }</a>
	                        		</c:if>
	                        	</c:forEach>
	                        	<c:if test="${not empty profilePageBoard.tags }">
	                        		과 함께
	                        	</c:if>
	                        	</div>
	                        </div>
	                        <div>
	                        	<c:if test="${not empty profilePageBoard.boardMedias}">
	                        		<c:forEach var="profilePageBoardMedia" items="${profilePageBoard.boardMedias }">
		                        		<img src="resources/image/${profilePageBoardMedia.name }" style="width:535px; height:350px;">
		                        	</c:forEach>
	                       		</c:if>
	                        </div>
	                      </div>
		                 <c:if test="${profilePageBoard.share.no ne null }">
		                     ...
		                     <div class="community-counter">
		                        <a href="javascript:void(0)" id="likeCount-${profilePageBoard.no }">
		                          <i class="fa fa-thumbs-up"></i>
		                          <span>${profilePageBoard.like }</span>
		                        </a>
		                        <a href="javascript:void(0)">
		                          <i class="far fa-comment-alt"></i>
		                          <span id="boardRepCount-${profilePageBoard.no }">${profilePageBoard.repCount }</span>
		                        </a>
		                    </div>
		                    <div class="newpostfooter">
		                       <div class="newpostfooter-contents">
		                       <!-- 좋아요를 눌렀는지아닌지에대한 choose -->
			                        <a href="javascript:void(0)" id="likeBoardButton-${profilePageBoard.no }" style="color:${profilePageBoard.event.user.no eq null ? '' : 'Dodgerblue' }">
			                          <label class="board-label"><i class="fa fa-thumbs-up"></i>좋아요</label>
			                        </a>
		                        <a href="javascript:void(0)" id="replyButton-${profilePageBoard.no }">
		                          <label class="board-label"><i class="far fa-comment-alt"></i>댓글달기</label>
		                        </a>
		                        <a id="shareButton-${profilePageBoard.no }" href="#" data-toggle="modal" data-target="#shareModal">
		                          <label class="board-label"><i class="fa fa-share"></i>공유하기</label>
		                        </a>
		                        </div>
		                      </div>
		                      <div id="commentListDiv-${profilePageBoard.no }">
		                      
		                      </div>
		                      
								
								<div class="commentpost" id="commentTextareaDiv-${profilePageBoard.no }" style="display:none">
								     <div class="input-group" style="width:100%">
								     	<div style="display:inline-block">
									       <a href="profileOpen.do?userNo=${profilePageBoard.user.no }">
								       ...
								       <textarea class="form-control" placeholder="댓글을 입력해주세요" id="replyContents-${profilePageBoard.no}" style="height:35px;"></textarea>
								       </div>
								       <div style="display:inline-block; vertical-align:middle;">
								       <a class="btn btn-primary btn-sm pull-right" id="addReply-${profilePageBoard.no }">등록</a>
								       </div>
								      </div>
								</div>
	                  </div>
		...
		<!-- 글쓰기 모달 -->
			...
			<div class="modal-dialog modal-center modal-overall">
				<div class="modal-content">
					<form method="post" id="board-form" action="addProfileBoard.do" enctype="multipart/form-data">
					<div class="modal-header">
		...
		var html = "";
		 $.ajax({
				...
				success: function(reply) {
					...
					html +=	'<img src="resources/image/'+reply.user.profile+'" />';
					html +=	'</a>';
					html += '<a href="" class="btn btn-danger btn-xs">신고하기</a>';
					html +=	'<div class="well well-sm">'+reply.user.name+':'+reply.contents+'</div>';
					html +=	'</div>';
					html += '</div>';				
