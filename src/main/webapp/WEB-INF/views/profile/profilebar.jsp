...
<div class="row">
	<div class="col-lg-10">
		<div class="header" style="background-image: url(resources/image/${user.cover})">
			<input type="hidden" id="hidden-no" value=${user.no }>
			<form method="post" id="form" action="addcover" enctype="multipart/form-data">
					<div>
					<c:if test="${LOGIN_USER.no == user.no }">
						<img id="blah">
			...
			<form method="post" id="form2" action="addprofile" enctype="multipart/form-data">
				<div class="image-profile">
					<img src="resources/image/${user.profile}" id="profile" ${LOGIN_USER.no == user.no ? '':'style="margin-top:50px;'}">
				</div>
				<c:if test="${LOGIN_USER.no == user.no }">
				...
				<div class="pull-right right-button">
						<c:choose>
							...
							<c:if test="${loginUser.no != checkNo}">
								<span id="follow-area">
			...
			<c:if test="${user.available eq 'N' }">
				<div class="text-right">
					<p style="font-size:15px; color:red; margin-right:150px; margin-top:20px;"> 이 사용자는 정지된 사용자 입니다.</p>
				</div>
			</c:if>
